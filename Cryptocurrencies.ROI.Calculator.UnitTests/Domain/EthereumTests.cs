...
namespace Cryptocurrencies.ROI.Calculator.UnitTests.Domain
{
    ...
    {
        [Fact]
        public void Given_AllValidParameters_When_EthereumCreated_Then_ObjectInstantiated()
        {
            //Arrange
            ...
            //Act
            var ethereum = new Ethereum(currencyValue, blockReward, networkDifficulty);
            //Assert
            Assert.Equal("Ethereum", ethereum.CurrencyName);
            Assert.Equal(currencyValue, ethereum.CurrencyPrice);
            Assert.Equal(blockReward, ethereum.BlockRewards);
            Assert.Equal(networkDifficulty, ethereum.NetworkDifficulty);
